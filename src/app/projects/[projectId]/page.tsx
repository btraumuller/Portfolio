...
export default async function projectDetail({params}:{
    ...
    let projectDetail:projectDetailData = await GetStaticProps(params.projectId);
    ...
    if(projectDetail){
        return(
            <main>
                ...
                <div className="py-4"><Link className="flex-sm flex" href="/projects"><FontAwesomeIcon icon={faChevronLeft} className="self-center mr-2 text-l" />Back to Projects</Link></div>
                <h1 className="text-center text-2xl lg:text-5xl mb-8">{projectDetail.title}</h1>
                <Image className="detail-image" src={projectDetail.projectFields.desktopImage.node.mediaItemUrl} width={800} height={800} alt={projectDetail.projectFields.desktopImageAltText}/>
                <div className="py-8">
                    <p className="py-4 text-lg lg:text-xl"><b>Created:</b> {datePosted}</p>
                    <p className="py-4 text-lg lg:text-xl" dangerouslySetInnerHTML={editorContent} />
                </div>
